...
Check that all the *na*s have been removed and look at the data frame.
```{r}
for (i in 1:13) {
...
bike_share_2023_cleaned <-  bike_share_2023_cleaned %>% 
  filter(start_station_id != "OH Charging Stx - Test" | end_station_id != "OH Charging Stx - Test")
```
Next the times are checked to make sure that the ended_at times are all later than the corresponding started_at times. This should only be possible on 2023-11-05 when the daylight saving time ended and the clocks went backward.
```{r}
bike_share_2023_cleaned %>% 
